...
By default the live probe stops after the first `GEMINI_HTTP_429` response so rehearsal does not keep spending failed quota calls. Use `-ContinueOnQuotaLimit` only when you intentionally want the full fallback transcript.

Observed live Gemini samples are recorded in `docs/live_gemini_test_report.md`.
