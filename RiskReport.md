...
| In-memory session storage | Medium | Sessions reset when server restarts or Cloud Run instance restarts | Restart loses current training session, though same-instance refresh can recover route/transcript | Keep 3-minute demo linear; Firestore optional only if persistence becomes required | Accepted for MVP |
