...
| Firestore persistence issues | Low | Potential connection drop or write failure to Google Cloud Firestore | API throws 5xx error in production (strict mode) to prevent silent loss of data; local runs fallback to in-memory | Mitigated via Firestore Store wrapper and strict error policy | Mitigated |
| Concurrency lock leakages | Low | Process crash during a chat turn leaving `isProcessing` flag locked | Session remains locked forever in Firestore | Implemented 30-second TTL expiration on the `isProcessing` lock in `chatOrchestrator.js` | Mitigated |
