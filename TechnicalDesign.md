...
| Frontend | React generated by AI Studio | AI Studio web app default; dùng đơn giản cho 4 màn hình MVP |
| Server runtime | Node.js server-side runtime in AI Studio | Gọi Gemini API từ server, giữ API key không lộ client |
...
Fallback local only: Python FastAPI + SQLite có thể dùng nếu AI Studio bị hạn chế, nhưng không phải hướng chính.
  ...
  "detectedEvents": [
    {
      ...
      "status": "triggered",
      "evidenceText": "Ngân hàng cần xác minh ngay..."
    }
  ],
  "safety": {
  ...
  "missedRedFlags": [
    {
      ...
      "recommendation": "Gọi lại số hotline chính thức in trên thẻ hoặc app ngân hàng."
    }
  ],
